...
class Reader(common.BinaryReader):
    ...
    def read_bone_frame(self):
        ...
        # complement data
        # frame.complement= ''.join(['%x' % x for x in struct.unpack('64B', self.ios.read(64))])
        frame.complement = [int(x) for x in struct.unpack('64B', self.ios.read(64))]
        return frame
